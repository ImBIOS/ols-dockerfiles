...
push_image() {
  ...
  if [ "${PUSH}" = true ]; then
    [ -f ~/.docker/imbios/config.json ] && CONFIG="--config ~/.docker/imbios"
...
# Cleanup previous build containers
cleanup_containers() {
  local version=$1
  local php_version=$2
  local suffix=$3

  echo "Cleaning up previous containers for ${BUILDER}/${REPO}:${version}-${php_version}${suffix}..."
  local container_ids
  container_ids=$(docker ps -a -q --filter "ancestor=${BUILDER}/${REPO}:${version}-${php_version}${suffix}")
...
main() {
  ...
  # Cleanup containers for the base version
  cleanup_containers "${OLS_VERSION}" "${PHP_VERSION}" ""
